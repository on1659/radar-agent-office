...
> **이관됨**: `working` 상태 배지 펄싱 애니메이션 (CSS keyframe) → **Phase 1.5** (Gap analysis: 색상 구분으로 대체 가능, Phase 1 기능 동작에 영향 없음)
...
| `working` 상태 시각적 피드백 강화 (펄싱 애니메이션) | Phase 1.5 | Gap analysis 이관: CSS keyframe 미구현 — 색상 구분으로 대체 가능. 기능 동작에 영향 없어 후순위 |
| 접근성 고려사항 (ARIA labels, keyboard navigation) | Phase 1.5 | Gap analysis 이관: 기능 MVP 완료 후 적용 — 스크린리더, 키보드 전용 사용 시나리오 포함 |
...
| 0.4 | 2026-04-08 | Section 10.5 Gap analysis 이관: 펄싱 애니메이션 Phase 1 성공 기준에서 제거 → Phase 1.5로 이관. Section 10.6 Phase 1.5 백로그 2개 추가: (1) working 상태 펄싱 애니메이션 (2) 접근성 고려사항(ARIA labels, keyboard navigation). | planner |
